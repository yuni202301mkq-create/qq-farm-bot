...
抓包服务（Code/GID 抓取）默认开启：源码运行、Docker 和二进制发布版都会随服务自动启动，
只需代理端口 `18000`。如需关闭，可在“系统配置 → Code/GID 抓取服务”中手动关闭。
...
日志本身始终是 UTF-8，乱码是查看终端解码不对或 ANSI 颜色码被错误渲染造成的，按下面任选即可：

```bash
# Linux 服务器（SSH 会话内执行后再启动服务）
export LANG=C.UTF-8 LC_ALL=C.UTF-8

# Windows 服务器（CMD/PowerShell）
chcp 65001
```

另外检查 SSH 客户端（Xshell、宝塔终端等）的字符编码设置为 UTF-8；程序输出到文件或
systemd/docker 日志时不会带颜色码，重定向查看不会再出现 `[t'o`、`¶` 之类的乱码。
