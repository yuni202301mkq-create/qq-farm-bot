...
- 卡密已按用途拆成两类，**一类卡密只带一种效果，互不混用**（`user-store.js` 的 `CARD_KEY_TYPE_DURATION` / `CARD_KEY_TYPE_QUOTA`）：`duration` 时效卡密只延长有效期，注册页与登录后都能激活；`quota` 额度账号卡密只增加账号数量上限，**必须在登录后于应用内激活**。`createCardKeys` 按 `type` 把另一项固定写 0；`loadCardKeys` 用 `normalizeCardKey` 归一化，旧记录无 `type` 时按「先看 days、再看 accountLimit」推断归属并把非本类型字段清零（历史组合卡按时效卡处理）。登录前路径 `/api/register`、`/api/renew` 改用 `consumeCardKeyBeforeLogin`：**先校验类型再消耗**，顺序不能反——先消耗会把用户手里的额度卡白废掉；额度卡被拒时返回「额度账号卡密只能在登录后激活，请先登录再使用」且卡密保持未使用。登录后 `/api/user/card-redeem` 是额度卡唯一入口，按 `type` 只应用对应效果，不会互相串味；`/api/user/card-inspect`、`/api/card-keys` 列表、生成接口均透出 `type` / `typeText`。额度不再通过卡密发放，注册额度统一走 `createUser` 的默认值（现为 1）。前端：`CardKeyPanel.vue` 生成区加类型下拉并按类型只显示「有效天数」或「账号数上限」、列表加类型列；`AuthView.vue` 注册/续费文案明确「此处仅接受时效卡密」；`RenewCardModal.vue` 改为「激活卡密」并按时效/额度分别展示；`MyCardKeyCard.vue` 按类型汇报激活结果。测试见 `core/test/card-key-types.test.js`（16 用例）。注意：`/api/forgot-password/verify` 仍可用已激活的额度卡密作为身份凭证找回密码（只做绑定校验、不触发激活），行为与时效卡一致。
...
- 注册页「免费领取 7 天卡密」：公开接口 `POST /api/free-card`（`admin-auth-routes.js`）发放 7 天时效试用卡（不带额度）并自动填入卡密框；领取记录持久化在 `data/free-card-claims.json`（`user-store` 的 `claimFreeCard` / `findFreeCardClaim` / `listFreeCardClaims` / `resetFreeCardClaims` / `normalizeClaimAddress`），**同一来源地址只能领一次**，来源取 `req.socket.remoteAddress`（不可用 `req.ip`，会被 XFF 伪造），`::ffff:` 前缀会归一化。超管可用 `GET /api/free-card-claims` 查看、`POST /api/free-card-claims/reset`（带 address 清单个，不带清全部）处理 NAT/共享出口被占用的情况。测试见 `core/test/free-card.test.js`（10 用例）。
...
- 已完成前端全站 ESLint 清理：对 `web/src` 全量 `eslint --fix` 收敛缩进/换行/CSS 与 UnoCSS class 排序/import 顺序等自动可修项，并手工修复 4 处无法自动修复的问题（`AccountModal.vue` 的 `stopWxCheck` 定义顺序、`CharityFlowerActivityPanel.vue` 单行双语句、`StrategyTimingPanel.vue` 未使用的 `props`）；`web/src` 全量 ESLint 现为 0 error 0 warning，`web` 生产构建通过。**（该结论已过时）** 后续改动后 `web/src` 全量 ESLint 现为 5 error / 53 warning（均为与卡密改造无关的既有问题），生产构建仍通过。
