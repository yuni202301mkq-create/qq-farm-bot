...
// 多实例场景（一页几十个开关）每个实例的 checkbox 需要唯一 id，供 label[for] 关联
const inputId = `plane-toggle-${Math.random().toString(36).slice(2, 10)}`
...
<template>
  ...
  >
    <!-- From Uiverse.io by Nawsome（适配为 BaseSwitch） -->
    <label class="plane-switch" :for="inputId">
      <input
        :id="inputId"
        type="checkbox"
        :checked="!!model"
    ...
    <label v-if="label" :for="inputId" class="select-none text-sm text-gray-700 font-medium dark:text-gray-300">
      {{ label }}
    </label>
  </div>
</template>
...
<style scoped>
/* ===== 飞机起飞开关（来源 Uiverse.io by Nawsome） =====
   原生 50x25 紧凑尺寸，桌面/移动端免缩放；灰=关（街道），蓝=开（起飞）。 */
.plane-switch {
  --dot: #fff;
  --street: #6b6d76;
  --street-line: #a8aab4;
...
.plane-switch input {
  /* 不用 display:none：保留键盘可达性，视觉上完全隐藏 */
  position: absolute;
  opacity: 0;
  width: 0;
  height: 0;
...
/* ===== 可访问性与适配 ===== */
/* 键盘聚焦时给开关一个可见描边（input 保留可聚焦，不用 display:none） */
.plane-switch input:focus-visible + div {
